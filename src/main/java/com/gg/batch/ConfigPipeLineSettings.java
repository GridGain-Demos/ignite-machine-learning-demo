...
public class ConfigPipeLineSettings {
	...
	public static  String FILEPREFIX = null;   // set this once only first time called
	
	public static Boolean statusFileOpenedSuccessfully = false;
    ...
    public static Properties MLPIPELINEPROPS = null;
	  ...
	  public static void main(String args[]) throws IOException {
	      ...
	      System.out.println(PROPERTIESFROMFILE);
   ...
   public static void checkDataSplitSettings() {
	...
	static int statusFileWriterInit() {
		
    	
		if (statusFileOpenedSuccessfully == true)
			return 0;
		...
		if (fileprefix.isBlank())  // if blank set to timestamp
    	...
    	String filepath =  outputDir + FILEPREFIX + "-batch-status.txt";   // default name with timestamp
    	
		int   fail = 0;
		try {
    		statusOutputFile  = new File(filepath);
    		System.out.println("Output file = {" + statusOutputFile.getAbsolutePath() + "}");
    		statusFileWriter = new FileWriter(statusOutputFile, false);
    	}
    	catch (Exception e) {
	...
	static void statusFileWriteMsg (String contents)
	{
	...
	static void statusFileWriterFlushAndClose () {
